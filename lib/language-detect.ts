/**
 * Lightweight V1 language detection for wild-word cleanup and display fixes.
 ...
 * `extensions/lenguariver-extension/lib/language-detect.ts` for everything through
 ...
 * - Run `npm run verify:language-detect-sync` after every change.
 * - Web-only below: `resolveWildWordDetectLanguage` (cleanup; no settings fallback).
 * - Extension-only (other file): `resolveSaveLanguage`, `emergencySaveLanguageAfterDetectorFailure`.
 */
 ...
 */
export function resolveSelectedTextLanguage(text: string): LanguageDetectionResult {
...
/**
 * Resolve language for My Words cleanup: selected text first, then context.
 * No settings fallback — returns `null` when confidence is low.
 */
export function resolveWildWordDetectLanguage(
  text: string,
  contextSentence?: string,
): LanguageDetectionResult | null {
  const tokenDetection = resolveSelectedTextLanguage(text);
